...
export class AppointmentReportComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.topicSubscription = this.stompService.watch('/topic/appointment-report/status').subscribe((msg: Message) => {
            if (msg.body === 'start') {
                this.running = true
    ...
    convert() {
        this.running = true;
        this.finished = false;
        this.service.appointmentReport({
            start: this.dateRange.start,
            end: this.dateRange.end,
            type: this.type,
            facilityId: this.facility.id
        }).subscribe((res) => {
            const file = new File([res], this.facility.name + '_' + ' Appointment Report.pdf', {type: 'application/octet-stream'});
            saveAs(file);
        })
    }
